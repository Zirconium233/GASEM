...
from network.flownet3d_utils import PointNetSetAbstraction,PointNetFeaturePropogation,FlowEmbedding,PointNetSetUpConv
...
class FlowNet3D(nn.Module):
    def __init__(self, in_channel=3, out_channel=3):
        ...
        # edit the parameters of the layers so that we can change the number of input and output channels
        self.sa1 = PointNetSetAbstraction(npoint=1024, radius=0.5, nsample=16, in_channel=in_channel, mlp=[32, 32, 64], group_all=False)
        ...
        self.fp = PointNetFeaturePropogation(in_channel=256+in_channel, mlp=[256, 256])
        
        self.conv1 = nn.Conv1d(256, 128, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm1d(128)
        self.conv2 = nn.Conv1d(128, out_channel, kernel_size=1, bias=True)
...
if __name__ == '__main__':
    ...
    output = model(input,input,feat,feat)
    print(output.size())
