...
def sample_and_save(filename, data_path, save_path, num_points, visualize=False):
    ...
    scale_param = [max_radius, center[0], center[1], center[2]]
    ...
    metafile['scale_param'] = scale_param
    # np.savetxt(pjoin(meta_save_path, filename + '.txt'), scale_param, delimiter=',')
    with open(pjoin(meta_save_path, filename + '.json'), 'w+', encoding='utf-8') as f:
        f.write(json.dumps(metafile))
        f.flush()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='../render_tools/test_rendered', help='Specify the path to the rendered data')
