...
from utils.config_utils import PARTNET_DATASET_PATH, AKB48_DATASET_PATH, PARTNET_ID_PATH, AKB48_ID_PATH, PARTNET_CAMERA_POSITION_RANGE, \
    AKB48_CAMERA_POSITION_RANGE, TARGET_GAPARTS, BACKGROUND_RGB, SAVE_PATH, FIX_CAMERA
...
from utils.render_utils import get_cam_pos, get_cam_pos_fix, set_all_scene, render_rgb_image, render_depth_map, \
...
def render_one_image(dataset_name, model_id, camera_idx, render_idx, height, width, use_raytracing=False, replace_texture=False):
    ...
    if not FIX_CAMERA:
        ...
        # defaultly set the camera position to the center of the range
        camera_pos = get_cam_pos_fix(
            theta=camera_range['theta_min'] + (camera_range['theta_max'] - camera_range['theta_min']) / 2,
            phi=camera_range['phi_min'] + (camera_range['phi_max'] - camera_range['phi_min']) / 2,
            distance=camera_range['distance_min'] + (camera_range['distance_max'] - camera_range['distance_min']) / 2
        )
    ...
    link_pose_dict = query_part_pose_from_joint_qpos(data_path=data_path, anno_file='link_anno_gapartnet.json', joint_qpos=joint_qpos, joints_dict=joints_dict, target_parts=TARGET_GAPARTS, base_link_name=base_link_name, robot=robot)
    ...
    if replace_texture:
        ...
        texture_joints_dict = read_joints_from_urdf_file(data_path, 'mobility_relabel.urdf')
